const fs = require('fs');
const appJsPath = 'c:\\Users\\USER\\Downloads\\USTC_learnX\\src\\main\\resources\\static\\app.js';
let appJs = fs.readFileSync(appJsPath, 'utf8');

// 1. Modify loadNotesLibrary for dynamic sorting
...
appJs = appJs.replace(oldNotesLib, newNotesLib);
...
function renderRoutineManager(container) {
    ...
    container.innerHTML = \``;
appJs = appJs.replace(oldRoutineMgrStart, newRoutineMgrCode);

// Add renderStudentSchedule
...
const newExamsHtml = `        <div class="glass-panel section-card">
            ...
            <div class="routine-table-container">`;
appJs = appJs.replace(oldExamsHtml, newExamsHtml);
...
const newLoadExamsListCall = `    document.getElementById('tab-active-exams').addEventListener('click', (e) => {
    ...
    loadExamsList('ACTIVE');\n}`;
appJs = appJs.replace(oldLoadExamsListCall, newLoadExamsListCall);
...
const newLoadExamsFuncStart = `async function loadExamsList(tab = 'ACTIVE') {
    ...
    if (list.length === 0) {`;
appJs = appJs.replace(oldLoadExamsFuncStart, newLoadExamsFuncStart);
